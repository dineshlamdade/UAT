...
export class SdmStepperComponent implements OnInit {
  ...
  derivedApplicableFlag: any = 1;
  totalMatrixRecords: any = 10
  ...
  stepperFunction(value) {
    ...
    this.summeryFlag = !this.summeryFlag;
    this.matrixsdmSourceCombinationList = []
  ...
  activeSource() {
    this.sdmService.activeSource(this.sdmMasterId).subscribe(res => {
      this.sourceCombinationData1 = res.data.results[0];
      this.sourceFieldListData1 = this.sourceCombinationData.sourceFieldList;
      this.sdmSourceCombinationListData = this.sourceCombinationData1.sdmSourceCombinationList;
      this.srcCombLength = this.sdmSourceCombinationListData.length
      this.SelectedCount = 0
      this.sdmSourceCombinationListData.forEach(element => {
        if (element.active) {
    ...
    this.sdmService.derivedTablesFields().subscribe(res => {
      ...
      res.data.results.forEach(element => {
        if (this.derivedTypeName == 'JobField') {
          ...
          if (element.derivedObjectName == 'EmployeeJobMapping' || element.derivedObjectName == 'EmployeePositionMapping' || element.derivedObjectName == 'WorkflowMaster' ||
          ...
          if (element.derivedObjectName != 'EmployeeJobMapping' && element.derivedObjectName != 'EmployeePositionMapping' &&
            ...
            && element.derivedObjectName != 'PositionDetailDD') {
            if (element.derivedObjectName != 'WorkflowMaster') {
              this.derivedTableFieldsData.push(element)
            }
          }
        }
  ...
  derivedTableNameChange(value) {
    ...
    if (this.derivedTableName == 'NonRecurringTransactionGroup' ||
      this.derivedTableName == 'GarnishmentMaster' ||
      this.derivedTableName == 'NonRecurringQuantity' || this.derivedTableName == 'FinancialMaster') {
      const formData = new FormData();

      let name = ''
      if (value == 'GarnishmentMaster') {
        name = 'Non-Recurring-Garnishment'
      }
      else if (value == 'NonRecurringTransactionGroup') {
        name = 'Non-Recurring-Amount'
      }
      else if (value == 'NonRecurringQuantity') {
        name = 'Non-Recurring-Quantity'
      }
      else if (value == 'FinancialMaster') {
        name = 'FinancialMaster'
      }
      formData.append('categoryName', name)
      this.sdmService.payrollheadmaster(formData).subscribe(res => {
        this.headList = res.data.results
      })
    }
  ...
  onChangeDriveName(value) {
    ...
    this.tempSelectedDerivedName = value
...
this.matrixsdmSourceCombinationList.forEach(element => {
  element.disableFlag = false;
    ...
    // console.log(JSON.stringify(this.matrixsdmSourceCombinationList))

    // alert(value)
    if (value == 'No Derived') {
      this.noDerivedFlag = true;
    } else {
      this.noDerivedFlag = false;
    }
    // this.selectedDerivedName = value
    this.derivedMastersData.forEach(element => {
      if (element.derivedName == value) {
        ...
        this.sdmService.SDMCombinationwithDerivedId(formdata).subscribe(res => {
          ...
          this.SDMCombinationwithDerivedIdData.forEach(element => {
            ...
            this.tempEditMatrixData.push({
              ...
              "derivedFromDate": new Date(element.derivedFromDate),
              "derivedToDate": new Date(element.derivedToDate),
          ...
          //  this.totalMatrixRecords = this.tempEditMatrixData.length
          ...
          //this.tempEditMatrixData.Sort((a,b)=>a.sdmSourceCombinationId - b.sdmSourceCombinationId)
        ...
        if (element.derivedObjectName == 'JobMaster') {
          this.sdmService.derivedFieldName('JobMaster', element.derivedFieldName).subscribe(res => {
        ...
        //
          ...
          this.sdmService.derivedFieldName('complianceSDMMapping', 'complianceMasterId').subscribe(res => {
            this.derivedDropdownValue = res.data.results[0]
          })
          this.showDropdown = true;
        ...
        if (element.derivedObjectName == 'EmployeeMaster') {
          this.sdmService.derivedFieldName('EmployeeMaster', element.derivedFieldName).subscribe(res => {
            this.derivedDropdownValue = res.data.results[0]
          })
    ...
    if (this.saveMatrixData.length > 0) {
      this.saveMatrixData.forEach(element => {
        if (this.matrixDerivedMasterId == element.sdmDerivedMasterId) {
          if (element.sdmSourceCombinationId == srcCombData.sdmSourceCombinationId) {
            if (parseFloat(value) <= parseFloat(element.sourceRangeFrom) || parseFloat(value) <= parseFloat(element.sourceRangeTo)) {
              // && 
  ...
  sourceRangeToData(value, index, srcCombData) {
    ...
    if (this.saveMatrixData.length > 0) {
      this.saveMatrixData.forEach(element => {
        if (this.matrixDerivedMasterId == element.sdmDerivedMasterId) {
          if (element.sdmSourceCombinationId == srcCombData.sdmSourceCombinationId) {
            if (parseFloat(value) <= parseFloat(element.sourceRangeTo)) {
  ...
  getEditedSourceRangeFrom(value, srcCombData) {
    if (srcCombData.rangeApplicable == false) {
      this.cycle = this.cycleIdChanges(srcCombData.payrollAreaId);
    } else {
      this.cycle = this.cycleIdChanges(srcCombData.selectedCombData.payrollAreaId);
...
"periodName": srcCombData.periodName
...
"periodName": srcCombData.periodName,
  ...
  getEditFromDateForSave(value, srcCombData) {
    let valueCheck = srcCombData.hasOwnProperty('rangeApplicable');
    if (srcCombData.rangeApplicable == false) {
      this.cycle = this.cycleIdChanges(srcCombData.payrollAreaId);
    } else {
      this.cycle = this.cycleIdChanges(srcCombData.selectedCombData.payrollAreaId);
...
"periodName": srcCombData.periodName,
  ...
  deleteSourceDerive(data) {
    this.sdmDerivedMasterId = data.sdmDerivedMasterId;

    this.sdmService.deleteSourceDerive(this.sdmDerivedMasterId).subscribe(res => {
  ...
  applicableValueData(value, srcCombData, index) {
    ...
    this.matrixsdmSourceCombinationList[index].applicableValue = value
    if (this.sourceRangeFrom != '' && this.sourceRangeTo != '' && this.derivedFromDate != '' && this.derivedToDate != '' && this.applicableValue != '') {
      this.addbtnflag = true
    } else {
      this.addbtnflag = false
  ...
  getFromDateForSave(event, srcCombData, index) {
    ...
    //   }
    // } else {
    //   if (srcCombData.sourceRangeFrom != null) {
    //     this.cycle = this.cycleIdChanges(srcCombData.selectedCombData.payrollAreaId);
    //   } else {
    //     this.cycle = this.cycleIdChanges(srcCombData.payrollAreaId);
    //   }
  ...
  addMatrixData(sdmcombination, rowIndex) {
    ...
    if (this.selectedFromDateForSave != '') {
      fromdate = this.datepipe.transform(new Date(this.selectedFromDateForSave), 'yyyy-MM-dd')
    }
    let todate = ''
    ...
    if (this.selectedIndex == rowIndex) {
      this.saveMatrixData.push({
        ...
        "applicableValue": this.applicableValue,
        "periodName":sdmcombination.periodName,

        "inputCycleId": sdmcombination.inputCycleId
      })
      //change
      //fromdate = '';
      //change
  ...
  saveMatrix() {
    ...
    // if(!this.isRangeApplicableFlag && this.noDerivedFlag){
    //   this.matrixsdmSourceCombinationList.forEach(element => {
    //     this.saveMatrixData.push({
    //       "sdmCombinationId": "0",
    //       "sdmMasterId": this.sdmMasterId,
  ...
  getMatrixData() {
    this.sdmService.combinationMatrix(this.sdmMasterId).subscribe(res => {
      ...
      if (this.editFlag) {
        ...
        this.totalMatrixRecords = this.matrixsdmSourceCombinationList.length
  ...
  largepopup(template: TemplateRef<any>, index, data, type) {
  ...
  selectedEmpId(employeedata, event, index) {
    if (event.checked) {
      ...
      this.selectedEmployeeId = employeedata.employeeCode
      if (this.editFlag) {
        this.showEditTextBox = true
        // index && this.tempEditMatrixData.length > this.selectedIndex
      ...
      this.applicableValueData(this.selectedEmployeeId, this.selectedSercData, this.selectedIndex)
  ...
  cycleIdChanges(data) {
    ...
    frmData.append('payrollAreaId', data)
    this.sdmService.postCycleIdData(frmData).subscribe(res => {
